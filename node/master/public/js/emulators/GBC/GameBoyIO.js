...
var settings = [						//Some settings.
	...
	8,									//Interval for the emulator loop. (milliseconds)
...
var gbBufferedInputs = {};
var lastIntervalTime = 0;
var gbKeymap = ["right", "left", "up", "down", "a", "b", "select", "start"];	//Keyboard button map.
...
function run() {
	if (GameBoyEmulatorInitialized()) {
		if (!GameBoyEmulatorPlaying()) {
			...
			gameboy.lastIntervalTime = gameboy.firstIteration;
			gameboy.iterations = 0;
			gbRunInterval = setInterval(function () {
				if (!document.hidden && !document.msHidden && !document.mozHidden && !document.webkitHidden) {
					gameboy.run();
					var nd = (new Date()).getTime();
					var dt = nd - gameboy.lastIntervalTime
					gameboy.lastIntervalTime = nd;
					handleInputConversion( dt );
				}
			}, settings[6]);
			lastIntervalTime = gameboy.firstIteration
		}
		else {
...
function matchKey(key) {	//Maps a keyboard key to a gameboy key.
	...
	var keymap = gbKeymap;	//Keyboard button map.
...
//Returns the INTERNAL snapshot size
function getInternalCanvasSnapshot() {
	if (GameBoyEmulatorInitialized()) {
		return gameboy.canvasOffscreen;
	}
}
...
function loadState( d ) {
	if (GameBoyEmulatorInitialized()) {
		return gameboy.returnFromState( d );
	}
}

function setSpeed( s ) {
	if (GameBoyEmulatorInitialized()) {
		return gameboy.setSpeed( s );
	}
}

function gbPeekMemory( index ) {
	if (GameBoyEmulatorInitialized()) {
		if( index < 0 || index >= gameboy.memory.length ){
			return 0;
		}else{
			return gameboy.memory[ index ];
		}
	}
}

function gbWriteMemory( index, v ) {
	if (GameBoyEmulatorInitialized()) {
		if( index < 0 || index >= gameboy.memory.length ){
		}else{
			gameboy.memory[ index ] = v;
			return true;
		}
	}
	return false;
...
function gbMemorySize( ) {
	if (GameBoyEmulatorInitialized()) {
		return gameboy.memory.length;
	}
	return 0;
}


function handleInputConversion( dt )
{
	if (GameBoyEmulatorInitialized() && GameBoyEmulatorPlaying()) {
		for( var k in gbBufferedInputs ){

			if( gbBufferedInputs[ k ][0] <= 0 ){

				if( gbBufferedInputs[ k ][1] ){
					gbBufferedInputs[ k ][1] = false;
				...
				gbBufferedInputs[ k ][0] -= dt;
				if( gbBufferedInputs[ k ][0] <= 0 ){
					gbBufferedInputs[ k ][1] = true;
				}
			}
		}
	}
...
function gbHoldInput( key, nseconds ){
	if( nseconds == undefined ){ nseconds = 0; }
	if( nseconds < 0 ){
		if( gbBufferedInputs.hasOwnProperty( key ) ){
			if( gbBufferedInputs[ key ][0] <= 0 ){

				//gbBufferedInputs[ key ][1] = true;
			}else{
				gbBufferedInputs[ key ][0]  = -1;
				gbBufferedInputs[ key ][1] = true;
			}
		}
	}else{
		if( gbBufferedInputs.hasOwnProperty( key ) ){
			if( gbBufferedInputs[ key ][0] <= 0 ){
				gbBufferedInputs[ key ] = [ nseconds*1000, true ];
			}else{
				gbBufferedInputs[ key ][0] = Math.max( nseconds*1000, gbBufferedInputs[ key ][0] );
			}
		}else{
			gbBufferedInputs[ key ] = [nseconds*1000, true];
		}
...
function gbInputStackGet(){
	var R = [];
	for( var kk in gbKeymap ){
		if( gbBufferedInputs.hasOwnProperty( gbKeymap[kk] ) ){
			R.push( gbKeymap[kk] );
		}
	}
	return R;
